...
use crate::{Dofigen, DofigenContext, Error, Resource, Result, lock::LockFile};

pub fn get_file_path(path: &Option<String>) -> Result<String> {
...
pub fn get_lockfile_path(path: String) -> Option<PathBuf> {
    if path == "-" {
        None
    } else {
...
pub fn get_image_from_path(path: String, context: &mut DofigenContext) -> Result<Dofigen> {
    if path == "-" {
...
pub fn load_lockfile(path: Option<PathBuf>) -> Option<LockFile> {
    path.map(|path| {
        if path.exists() {
